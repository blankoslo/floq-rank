...
mkUser games name id =
  let
    mkProfile gameId = (Uuid.toString gameId, Profile 0 0 0 1000)
    profiles = Dict.fromList <| (List.map mkProfile (List.map .id games))
  in
    (id, User id name profiles)

mkGame : String -> Uuid -> (String, Game)
mkGame title uuid = (Uuid.toString uuid, Game uuid title)

type Rating = Rating Float
type Expected = Expected Float
...
newRating (Rating ra) (Expected ea) (Expected result) =
  let newRa = ra + kFactor * (result - ea)
  in Rating newRa

generateUuids : Seed -> Int -> List Uuid -> List Uuid
generateUuids seed n uuids =
    let (newUuid, newSeed) = step Uuid.uuidGenerator seed
    in generateUuids newSeed (n - 1) (newUuid::uuids)
...
employeesDecoder =
  let user = Decode.map3 FloqEmployee
...
fetchEmployees apiToken apiHost =
  let authHeader = Http.header "Authorization" ("Bearer " ++ apiToken)
  in Http.request
      { method = "GET"
      , headers = [authHeader]
...
gamesDecoder =
  let gameOrNothing a b = Maybe.map (\x -> Game x b) a
      game = Decode.map2 gameOrNothing
               (Decode.map Uuid.fromString (Decode.field "id" Decode.string))
               (Decode.field "title" Decode.string)
  in Decode.map (List.filterMap identity) (Decode.list game)
...
matchupDecoder =
  let matchupOrNothing a b c d e = Maybe.map2 (\ma me -> Matchup ma b c d me) a e
      toResult x = case x of
                     "WIN" -> Win
                     "LOSS" -> Loss
                     _ -> Tie
  in Decode.map5 matchupOrNothing
...
matchupsDecoder = Decode.map (List.filterMap identity) (Decode.list matchupDecoder)
...
fetchAll t h = Task.map3 (,,) (Http.toTask <| fetchEmployees t h)
                              (Http.toTask <| fetchGames t h)
                              (Http.toTask <| fetchMatchups t h)

matchupEncoder : Matchup -> Encode.Value
matchupEncoder matchup =
  let toResultString x = case x of
                           Win -> "WIN"
                           Loss -> "LOSS"
                           Tie -> "TIE"
  in Encode.object [ ("game_id", Encode.string <| Uuid.toString matchup.gameId)
...
postMatchup apiToken apiHost matchup =
  let authHeader = Http.header "Authorization" ("Bearer " ++ apiToken)
      preferHeader = Http.header "Prefer" "return=representation"
  in Http.request
      { method = "POST"
...
init flags =
  { app          = Ui.App.init
  , gameChooser  = Ui.Chooser.init [] "" ""
                   |> \x -> { x | closeOnSelect = True, searchable = True }
  , user1Chooser = Ui.Chooser.init [] "" ""
                   |> \x -> { x | closeOnSelect = True, searchable = True }
  , user2Chooser = Ui.Chooser.init [] "" ""
                   |> \x -> { x | closeOnSelect = True, searchable = True }
  , user1Checkbox = Ui.Checkbox.init True
                    |> \x -> { x | readonly = True }
...
userToItem user = Ui.Chooser.Item user.name user.id

gameToItem : Game -> Ui.Chooser.Item
gameToItem game = Ui.Chooser.Item game.title (Uuid.toString game.id)

addMatchup : Matchup -> Model -> Model
addMatchup matchup model =
  let updateProfile result newRating = Maybe.map (\x -> { x
                                                        | nMatches = x.nMatches + 1
                                                        , nMatchesWon = if result == Win then x.nMatchesWon + 1 else x.nMatchesWon
                                                        , nMatchesLost = if result == Loss then x.nMatchesLost + 1 else x.nMatchesLost
                                                        , rating = newRating
                                                        }
                                                 )
      updateUser result (Rating newRating) = Maybe.map (\x -> { x | profiles = Dict.update (Uuid.toString matchup.gameId) (updateProfile result newRating) x.profiles })
      getOppositeResult x = case x of
                              Win -> Loss
                              Loss -> Win
                              _ -> Tie
      getExpected x = case x of
                        Win -> Expected 1
                        Loss -> Expected 0
                        Tie -> Expected 0.5
      oldU1Rating = Dict.get matchup.user1Id model.users
                    |> Maybe.andThen (.profiles >> Dict.get (Uuid.toString matchup.gameId))
                    |> Maybe.map (.rating)
                    |> Maybe.withDefault 1000
      oldU2Rating = Dict.get matchup.user2Id model.users
                    |> Maybe.andThen (.profiles >> Dict.get (Uuid.toString matchup.gameId))
                    |> Maybe.map (.rating)
                    |> Maybe.withDefault 1000
      (u1Result, u2Result) = (matchup.result, getOppositeResult (matchup.result))
      (ra, rb) = (Rating oldU1Rating, Rating oldU2Rating)
      (ea, eb) = expected (ra, rb)
      (u1Rating, u2Rating) = (newRating ra ea (getExpected u1Result), newRating rb eb (getExpected u2Result))
  in { model
     | matchups = (matchup::model.matchups)
     , users = Dict.update matchup.user1Id (updateUser u1Result u1Rating) model.users
...
update msg model =
  case msg of
    ...
    Init initialData ->
      let employeeToUser games employee = mkUser games (employee.firstName ++ " " ++ employee.lastName) (toString employee.id)
          (users, games, matchups) = case initialData of
                                       Ok (us, gs, ms) ->
                                         let games = List.map (\x -> (Uuid.toString x.id, x)) gs |> Dict.fromList
                                             users = List.map (employeeToUser (Dict.values games)) us |> Dict.fromList
                                         in (users, games, ms)
                                       Err _ -> (Dict.empty, Dict.empty, [])
          initialModel = { model
                         | users = users
                         , games = games
      ...
      in ( List.foldl addMatchup initialModel matchups
         , Cmd.none
         )

    App act ->
      let (app, effect) = Ui.App.update act model.app
      in ({ model | app = app }, Cmd.map App effect)

    GameChooser msg -> ( { model
                         | gameChooser = Ui.Chooser.update msg model.gameChooser |> Tuple.first
                         }
                       , Cmd.none )

    User1Chooser msg ->
      let user1Chooser = Ui.Chooser.update msg model.user1Chooser
                         |> Tuple.first
          selectedUser = Maybe.withDefault "" (Ui.Chooser.getFirstSelected user1Chooser)
          selectableUsers = Dict.values model.users
    ...
    User2Chooser msg ->
      let user2Chooser = Ui.Chooser.update msg model.user2Chooser
                         |> Tuple.first
          selectedUser = Maybe.withDefault "" (Ui.Chooser.getFirstSelected user2Chooser)
          selectableUsers = Dict.values model.users
                            |> List.filter (.id >> ((/=) selectedUser))
                            |> List.map userToItem
      in ( { model
    ...
    User1Checkbox msg ->
      let user1Checkbox = Ui.Checkbox.update msg model.user1Checkbox |> Tuple.first
          user2Checkbox = Ui.Checkbox.setValue ((.value >> not) user1Checkbox) model.user2Checkbox
          tieCheckbox = Ui.Checkbox.setValue ((.value >> not) user1Checkbox) model.tieCheckbox
      in ( { model
    ...
    TieCheckbox msg ->
      let tieCheckbox = Ui.Checkbox.update msg model.tieCheckbox |> Tuple.first
          user1Checkbox = Ui.Checkbox.setValue ((.value >> not) tieCheckbox) model.user1Checkbox
          user2Checkbox = Ui.Checkbox.setValue ((.value >> not) tieCheckbox) model.user2Checkbox
      in ( { model
           | tieCheckbox = { tieCheckbox | readonly = .value tieCheckbox }
           , user1Checkbox = { user1Checkbox | readonly = .value user1Checkbox }
           , user2Checkbox = { user2Checkbox | readonly = .value user2Checkbox }
    ...
    User2Checkbox msg ->
      let user2Checkbox = Ui.Checkbox.update msg model.user2Checkbox |> Tuple.first
          user1Checkbox = Ui.Checkbox.setValue ((.value >> not) user2Checkbox) model.user1Checkbox
          tieCheckbox = Ui.Checkbox.setValue ((.value >> not) user2Checkbox) model.tieCheckbox
      in ( { model
           | user1Checkbox = { user1Checkbox | readonly = .value user1Checkbox }
           , user2Checkbox = { user2Checkbox | readonly = .value user2Checkbox }
           , tieCheckbox = { tieCheckbox | readonly = .value tieCheckbox }
    ...
    SubmitMatchup f ->
      let postMatchupCmd = Task.map f Date.now
      ...
      in (model, postMatchupCmd)

    AddMatchup matchup -> (addMatchup matchup model, Cmd.none)

rankingsRowView : (Int, String, Profile) -> Html.Html Msg
rankingsRowView (rank, name, profile) =
  let rankStr = toString rank
      nMatchesStr = toString profile.nMatches
      nMatchesWonStr = toString profile.nMatchesWon
      nMatchesLostStr = toString profile.nMatchesLost
      nMatchesTiedStr = toString (profile.nMatches - (profile.nMatchesWon + profile.nMatchesLost))
      ratingStr = toString <| round profile.rating
  in Html.tr
       []
...
rankingsView model =
  let gameId = Maybe.withDefault "" (Ui.Chooser.getFirstSelected model.gameChooser)
      users = Dict.values model.users
              ...
              |> List.reverse
      ranks = List.range 1 (List.length users)
      names = List.map .name users
      profiles = List.filterMap (Dict.get gameId << .profiles) users
      header = Html.thead
                 []
                 [ Html.tr
                     []
      ...
      rows = if (List.length profiles > 0) then
...
matchupRowView (name1, name2, result, createdAt) =
  let name1Elem = case result of
                    Loss -> text name1
                    _ -> strong [] [ text name1 ]
      name2Elem = case result of
                    Win -> text name2
                    _ -> strong [] [ text name2 ]
      createdAtStr = text <| Date.formatISO8601 createdAt
  in Html.tr
       []
       [ Html.td [] [ name1Elem ]
       , Html.td [] [ name2Elem ]
...
getName model x = Maybe.withDefault "" (Maybe.map .name (Dict.get x model.users))
...
matchupsView model =
  let gameId = Maybe.withDefault "" (Ui.Chooser.getFirstSelected model.gameChooser)
      ...
      matchups = model.matchups
                 ...
                 |> List.take (limit + 1)
      names1 = List.map (.user1Id >> getName model) matchups
      names2 = List.map (.user2Id >> getName model) matchups
      results = List.map .result matchups
      createdAts = List.map .createdAt matchups
      header = Html.thead
                 []
                 [ Html.tr
                     []
      ...
      rows = if (List.length results > 0) then
  ...
  in Ui.App.view
       ...
       , if List.length rows > limit then text "..." else text ""
       ]

getResult : (Bool, Bool, Bool) -> Result
getResult x = case x of
                (True, False, False) -> Win
                (False, False, True) -> Loss
                _ -> Tie

view : Model -> Html.Html Msg
view model =
  let gameId = Ui.Chooser.getFirstSelected model.gameChooser
               |> Maybe.map Uuid.fromString |> Maybe.join
      user1Id = Ui.Chooser.getFirstSelected model.user1Chooser
      user2Id = Ui.Chooser.getFirstSelected model.user2Chooser
      result = getResult (.value model.user1Checkbox, .value model.tieCheckbox, .value model.user2Checkbox)
      matchup = Maybe.map3 (\a b c -> SubmitMatchup (\date -> Matchup a b c result date)) gameId user1Id user2Id
      verb = case result of
               Win -> "wins"
               Tie -> "ties"
               Loss -> "loses"
      toMatchupStr a b = "" ++ (getName model a) ++ " " ++ verb ++ " against " ++ (getName model b) ++ "!"
      matchupStr = Maybe.map2 toMatchupStr user1Id user2Id
  in
    Ui.App.view
      App
      model.app
